...
export default function ServiceHighlights() {
  const services = [
    ...
    },
    {
      ...
      description:
        "Maximizing resources with strategic financial planning and analysis.",
      image:
  ...
  return (
    <div className="w-full bg-black rounded-t-[60px] py-20 px-8 md:px-16">
      {/* Header */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto">
        {services.map((service, index) => (
          ...
          >
            {/* IMAGE (one single image transforming) */}
            <img
              src={service.image}
              alt={service.title}
              className="
                absolute 
                top-0 right-0
                w-1/2 h-1/2                 /* Quadrant size */
                object-cover 
                rounded-bl-[150px]

                transition-all duration-700 ease-out

                group-hover:w-full          /* Expand to full card */
                group-hover:h-full
                group-hover:top-0
                group-hover:right-0
            ...
            {/* ICON */}
            <div className="absolute top-6 left-6 w-14 h-14 bg-green-400 rounded-full flex items-center justify-center text-black shadow-lg z-20">
              {service.icon}
            </div>

            {/* CONTENT */}
            <div className="relative z-20 p-6 pt-48">
